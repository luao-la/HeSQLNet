...
class MyAstor(object):
    '''
    root node必须只有一个, attr node顺序必须按索引排列的
    '''
    ...
    def _breadth_walk_func(self):#广度优先遍历
        """
        只广度遍历功能节点,输出id,内部函数
        ...
        """
        # root_node_id=self.get_root_index()
    ...
    # def _depth_walk_all(self,node_id):
    #     node_ids = [node_id]
    #     for child_id in self.get_child_ids(node_id):
    #         node_ids.extend(self._depth_walk_all(child_id))
    #     return node_ids
    #
    # def depth_walk_all(self):
    #     return self._depth_walk_all(self.root_id)
    ...
    def to_hetero(self,node_hetero=False,add_global_node=False):
        '''
        将AST转化为异构图
        :param node_hetero: 是否异构化节点
        :param add_global_node: 是否添加全局节点
        :return:
        '''
        ...
        func_id2id=dict()   #AST中功能节点id到异构图中功能节点的id
        attr_id2id = dict()  #AST中属性节点id到异构图中属性节点的id
        ...
        for i in range(self.edges.shape[1]):
            ...
            if child_id not in self.attr_node_ids:  #父节点肯定是功能节点，如果子节点也是功能节点
        ...
        func_child_func_edges=np.concatenate(func_child_func_edges,axis=0).T #shape of (2,edge_num)
        func_parent_func_edges=np.concatenate(func_parent_func_edges,axis=0).T #shape of (2,edge_num)
        func_child_attr_edges=np.concatenate(func_child_attr_edges,axis=0).T #shape of (2,edge_num)
        attr_parent_func_edges=np.concatenate(attr_parent_func_edges,axis=0).T #shape of (2,edge_num)
