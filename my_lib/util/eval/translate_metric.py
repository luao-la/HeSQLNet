...
from .nltk_bleu_score_3_3 import sentence_bleu, SmoothingFunction
...
def get_nltk33_sent_bleu1(preds,refs,auto_reweigh=True):
    ...
    scores=[sentence_bleu(ref,pred+[' ']*max(0,2-len(pred)),weights=(1.,0.,0.,0.),smoothing_function=cc.method4,auto_reweigh=auto_reweigh)
...
def get_nltk33_sent_bleu2(preds,refs,auto_reweigh=True):
    ...
    if not isinstance(refs[0][0], list):  # 必不可少
        refs = [[seq] for seq in refs]
    cc = SmoothingFunction()
    scores = [sentence_bleu(ref, pred+[' ']*max(0,2-len(pred)), weights=(0., 1., 0., 0.), smoothing_function=cc.method4,auto_reweigh=auto_reweigh)
              for pred, ref in zip(preds, refs)]
    return sum(scores)/len(scores)*100
...
def get_nltk33_sent_bleu3(preds,refs,auto_reweigh=True):
    ...
    if not isinstance(refs[0][0], list):  # 必不可少
        refs = [[seq] for seq in refs]
    cc = SmoothingFunction()
    scores = [sentence_bleu(ref, pred+[' ']*max(0,2-len(pred)), weights=(0., 0., 1., 0.), smoothing_function=cc.method4,auto_reweigh=auto_reweigh)
              for pred, ref in zip(preds, refs)]
    return sum(scores)/len(scores)*100
...
def get_nltk33_sent_bleu4(preds,refs,auto_reweigh=True):
    ...
    if not isinstance(refs[0][0], list):  # 必不可少
        refs = [[seq] for seq in refs]
    cc = SmoothingFunction()
    scores = [sentence_bleu(ref, pred+[' ']*max(0,2-len(pred)), weights=(0., 0., 0., 1.), smoothing_function=cc.method4,auto_reweigh=auto_reweigh)
              for pred, ref in zip(preds, refs)]
    return sum(scores)/len(scores)*100
...
def get_nltk33_sent_bleu(preds,refs,auto_reweigh=True):
    ...
    if not isinstance(refs[0][0], list):  # 必不可少
        refs = [[seq] for seq in refs]
    cc = SmoothingFunction()
    scores=[sentence_bleu(ref, pred+[' ']*max(0,2-len(pred)),weights=(0.25,0.25,0.25,0.25),smoothing_function=cc.method4,auto_reweigh=auto_reweigh)
            for pred,ref in zip(preds,refs)]
    return sum(scores)/len(scores)*100
