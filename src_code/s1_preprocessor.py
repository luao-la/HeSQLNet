...
def make_rev_dic(train_raw_data_path, valid_raw_data_path, test_raw_data_path, tech_term_path, rev_dic_path,
    ...
    if not os.path.exists(path_dir):
        os.makedirs(path_dir)
    with open(train_raw_data_path, 'r') as f1, open(valid_raw_data_path, 'r') as f2, open(test_raw_data_path,
                                                                                          'r') as f3:
    ...
    parser = MySitter(lan='Spider', lemmatize=False, lower=False, ast_intact=True, seg_attr=True, rev_dic=None,
                      user_words=USER_WORDS)  # 这里lower=False！！
    all_tokens = []
    ...
    for token in pbar:
        ...
        if seg_token != token:
            ...
            if end_s_flag and len(seg_tokens[-1]) > 1:
                ...
                sec_seg_token_dic[seg_tokens[-1][0].upper() + seg_tokens[-1][1:].lower() + 's'] = seg_tokens[-1][
                                                                                                      0].upper() + \
                                                                                                  seg_tokens[-1][
                                                                                                  1:].lower()
            ...
            pbar.set_description(
                '[Segment tokens: {}-th segmented: {}:::{}]'.format(len(seg_token_dic), token, seg_token))
        else:
                ...
                if min_sub_token_len > 1:
                    ...
                    for i, new_token in enumerate(new_tokens):
                        if new_token.lower() in benign_lower_tokens and new_token.lower() not in user_words:
                            ...
                            sec_seg_token_dic[new_token[0].upper() + new_token[1:].lower()] = seg_token[
                                                                                                  0].upper() + seg_token[
                                                                                                               1:].lower()
                            if i == 1:
                        ...
                        for i in range(0, len(sub_tokens) - 1):
                            ...
                            for mini_new_token in mini_new_tokens:
                                if mini_new_token.lower() in benign_lower_tokens and mini_new_token.lower() not in user_words:
                                    ...
                                    add_num += 1
                                    pbar.set_description(
                                        '[Segment additional tokens: {}-th segmented: {}:::{}]'.format(add_num,
                                                                                                       mini_new_token,
                                                                                                       mini_seg_token))
                                    # if seg_token.lower in ['Add res','Add ress']:
                                    #     print(seg_token)
                for sub_token in sub_tokens:
                    if len(sub_token) > 2 and sub_token[-1].lower() != 's':
                        ...
                        if mini_new_token.lower() in benign_lower_tokens and mini_new_token.lower() not in user_words:
                            ...
                            sec_seg_token_dic[mini_new_token[0].upper() + mini_new_token[1:].lower()] = sub_token[
                                                                                                            0].upper() + sub_token[
                                                                                                                         1:].lower()
                            sec_seg_token_dic[
...
def _truncate_ast_by_code_and_layout(parser, start_code_token_id, end_code_token_id, max_graph_size=np.inf,
    ...
    code_token_ids = list(_parser_dict['code_token_edges'][0, :]) + [
        _parser_dict['code_token_edges'][1, -1]]  # must add list
    ...
    if start_code_token_id > 0 or end_code_token_id < len(code_token_ids):
        ...
        while len(child_node_ids) > 0:
            ...
            wids = np.where(
                _parser_dict['ast_edges'][0, :] == child_node_ids[:, None])  # 超找当前child_node_ids在ast_edges[0, :]中的索引
    ...
    if start_code_token_id > 0:
        ...
        father_wids = np.argwhere(
            parser_dict['code_layout_edges'][1, :] == root_node_id)  # 注意这里是code_layout_edges不是ast_edges
    ...
    if len(keep_ast_node_ids) < len(_parser_dict['ast_nodes']):
        ...
        if renew_pos and (parser_dict['ast_edges'].min() > _parser_dict['ast_edges'].min() or start_code_token_id > 0):
            ...
            # 更新行号
            if parser_dict['code_token_poses'][0][0] > 0 or parser_dict['code_token_poses'][0][
                1] > 0:  # if the first line number is larger than 0
            ...
            for node_id, node_pos in enumerate(parser_dict['ast_node_poses']):
                ...
                if plus_zpos_child_ids and parser_dict['ast_node_poses'][plus_zpos_child_ids[0]][2] > 0:
                    for zpos, child_id in enumerate(plus_zpos_child_ids):
                        ...
                        parser_dict['ast_node_poses'][child_id][0], parser_dict['ast_node_poses'][child_id][1], zpos)
...
def tokenize_raw_data(raw_data_path, token_data_path, rev_dic_path,
    ...
    parser = MySitter(lan='Spider', lemmatize=True, lower=True, ast_intact=True, seg_attr=True, rev_dic=rev_dic,
...
def build_avail_data(token_data_path,
    ...
    :return:
    '''
    logging.info('########### Start building the train dataset available for the roberta-base ##########')
    ...
    logging.info('########### Finish building the train dataset available for the roberta-base ##########')
