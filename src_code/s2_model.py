...
from torch_geometric.nn import HeteroConv, GraphNorm, HANConv
import random
import numpy as np
import os
import logging
    ...
    def __init__(self,
        ...
        for _ in range(graph_gnn_layers):
            if graph_GNN == TransformerConv:
                ...
                metapath = (['node'], [('node', 'base_child', 'node'), ('node', 'base_father', 'node'),
                                       ('node', 'sibling_next', 'node'), ('node', 'sibling_prev', 'node'),
                                       ('node', 'dfg_next', 'node'), ('node', 'dfg_prev', 'node'),
                                       ('node', 'code_next', 'node'), ('node', 'code_prev', 'node')])
                gnn = HANConv(in_channels, out_channels, metapath, heads=1)
                # gnn = HeteroConv({
...
class Dec(nn.Module):
    def __init__(self,
        ...
        self.pos_encoding = PosEnc(max_len=text_max_len + 1, emb_dims=emb_dims, train=True, pad=True,
                                   pad_idx=kwargs['pad_idx'])  # 不要忘了+1,因为输入前加了begin_id
...
class TNet(BaseNet):
    ...
    def forward(self, code_graph):
        # code_graph : batch_data
        text_input = code_graph['text'].text_token_input.clone()
        del code_graph['text']
        graph_enc, graph_code_enc, code_src_map = self.enc_op(data=code_graph)  # 执行编码器
        text_output = self.dec_op(graph_enc=graph_enc, graph_code_enc=graph_code_enc,  # 执行解码器获得输出
        ...
        if self.train_mode:  # 如果进行训练
            ...
            for i in range(0, self.max_big_epochs):
                ...
                for j, batch_data in enumerate(pbar):
                    ...
                    pred_text_output = self.net(batch_data)  # 使用模型输出
                ...
                if i + 1 >= self.start_valid_epoch:
                    ...
                    #                                    increase_better=True,
                    #                                    last=False)  # 根据验证集loss选择best_net
                    if worse_epochs == 0:  # 保存 验证集的结果
                        model.generate_texts(code_graphs=valid_data['code_graphs'],
                                             text_dic=valid_data['text_dic'],
                                             res_path=res_path,
                                             gold_texts=valid_data['texts'],
                ...
                if self.beam_width == 1:
                    for i in range(self.text_max_len + 1):  # 每步开启
                        ...
                        batch_text_output.append(
                            pred_out[:, :, i].unsqueeze(-1).to('cpu').data.numpy())  # 将该步输出加入msg output
...
if __name__ == '__main__':
    ...
    with codecs.open(valid_token_data_path, 'r') as f:
        valid_token_data = json.load(f)

    with codecs.open(valid_raw_data_path, 'r') as f:
    ...
    model.fit(train_data=train_data,
              ...
              valid_token_data=valid_token_data,
    ...
    # test_eval_df = roberta-base.eval(test_srcs=test_data['code_graphs'],
    ...
    #
    # roberta-base.generate_texts(code_graphs=test_data['code_graphs'],
